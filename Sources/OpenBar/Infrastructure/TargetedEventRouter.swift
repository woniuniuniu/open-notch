...
/// Routes one tagged event through the owning process and the session stream.
/// Both hops are required for hosted status items on recent macOS releases.
enum TargetedEventRouter {
    ...
    static func route(_ event: CGEvent, through targetPID: pid_t, timeout: TimeInterval = 0.25) -> Bool {
        ...
        guard let processTap = CGEvent.tapCreateForPid(
            pid: targetPID,
